...
class db_mysql
{
	...
	public function _log(){
        $error = $this->error();
	    if (!empty($error[1])) {
	        $path = LOGS_PATH.'/sql_error/';
	        if(!is_dir($path)) mkdir($path, 0755, true);
	        $log = '<?php exit;?>'.DATETIMES.':'.$error[2].':'.$error[1].PHP_EOL;
	        file_put_contents($path .'error-'. DAYTIMES .'.php', $log, FILE_APPEND | LOCK_EX);
	    }
	}
	
	public function find($sql, $columns = '*')
	{
	    $query = $this->query($this->where_clause($sql));
	...
	protected function where_clause($where)
	{
		...
		// 自定义检查
        $where_clause = $this->check_sql($where_clause);
		return $where_clause;
	}
	...
	function check_sql($db_string, $querytype = 'select')
	{
	    ...
	    $old_pos = 0;
	    $pos = - 1;
	    $log_file = LOGS_PATH . '/sql_error/inject-'. date('Y-m-d') .'.log';
	    $userIP = isset($_SERVER["REMOTE_ADDR"]) ? $_SERVER["REMOTE_ADDR"] : '';
	    $getUrl = $this->GetCurUrl_MYSQL();
	
	    // 如果是普通查询语句，直接过滤一些特殊语法
	    if ($querytype == 'select') {
	        $notallow1 = "/[^0-9a-z@\._-]{1,}(sleep|benchmark|load_file|outfile)[^0-9a-z@\.-]{1,}/i";
	    ...
	    if ($fail) {
	        @file_put_contents($log_file, "$userIP||$getUrl||$db_string||$error\r\n", LOCK_EX | FILE_APPEND);
	        exit("<font size='5' color='red'>Safe Alert: Request Error!</font>");
	    }
	    return $db_string;
	}
	// 获得当前的脚本网址
	function GetCurUrl_MYSQL(){
